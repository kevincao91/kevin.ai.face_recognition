...
def vector_normalization(vector):
    ...
    zero_vector = torch.from_numpy(zero_np).float().type_as(vector).cuda()
    new_vector = torch.from_numpy(zero_np).type_as(vector).cuda()
    ...
    for epoch in range(cuda_opt.max_epoch):
        ...
        print(
            "Training: Epoch[{:0>3}/{:0>3}] Loss_Avg_Epoch: {:.4f}       Lr: {:.8f}".format(
                epoch + 1, cuda_opt.max_epoch, loss_avg_epoch, lr_now))
        ...
        if epoch % 1 == 0:
            ...
            print(
                "Validating: Epoch[{:0>3}/{:0>3}] Loss_Avg_Epoch: {:.4f} Accuracy:{:.4f}".format(
                    epoch + 1, cuda_opt.max_epoch, val_loss_avg, val_acc_avg))
